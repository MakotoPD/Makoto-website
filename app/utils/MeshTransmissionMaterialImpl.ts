// utils/MeshTransmissionMaterialImpl.ts
...
// Definicje typów, które mogą być przydatne
interface Uniform<T> {
  value: T
}
...
interface Shader {
  ...
  defines: { [key: string]: string | number | boolean } | undefined
}

export class MeshTransmissionMaterialImpl extends THREE.MeshPhysicalMaterial {
  uniforms: {
    // ... (skopiuj całą definicję uniforms z oryginalnego pliku)
  ...
  constructor(samples = 6, transmissionSampler = false) {
    ...
    this.onBeforeCompile = (shader: Shader) => {
      ...
      // ... (SKOPIUJ CAŁĄ ZAWARTOŚĆ METODY onBeforeCompile BEZ ZMIAN)
      // W tym miejscu wklej cały kod shaderów GLSL z oryginalnego pliku.
      // Zaczyna się od: if ((this as any).anisotropy > 0) ...
      // A kończy na: totalDiffuse = mix( totalDiffuse, transmission.rgb, material.transmission );\n`
    ...
    Object.keys(this.uniforms).forEach((name) =>
      Object.defineProperty(this, name, {
        get: () => (this.uniforms as any)[name].value,
        set: (v) => ((this.uniforms as any)[name].value = v),
      })
    )
  }
}